...
export function initFieldsFilter() {
        ...
        visibleCards.sort((a, b) => {
            ...
            switch (sort) {
                ...
                case "rating": {
                    const scoreA =
                        (fieldA.rating || 0) *
                        Math.log10((fieldA.votes || 0) + 1);

                    const scoreB =
                        (fieldB.rating || 0) *
                        Math.log10((fieldB.votes || 0) + 1);
                ...
                case "newest":
                    return (
                        new Date(fieldB.createdAt || 0) -
                        new Date(fieldA.createdAt || 0)
                    );
    ...
    const nearbyFromURL = params.get("nearby");
    ...
    if (sortFromURL && sortBy) {
        sortBy.value = sortFromURL;
    }

    // Nearby
    if (nearbyFromURL === "true" && useLocationToggle) {
        useLocationToggle.checked = true;
        localStorage.setItem("useLocationToggle", "true");
    }

    // Clean URL
    if (searchFromURL || sortFromURL || nearbyFromURL) {
